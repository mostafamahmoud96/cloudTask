...
class FileController extends Controller
{
    public function upload_file(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'file' => 'required|mimes:jpeg,pdf,png,csv'
        ]);
        if ($validator->fails()) {
            return response()
                ->json(['status' => 415, 'message' => 'You Must upload valid Type file']);
        }
        $fileModel = new File();
        ...
        if (File::where('name', md5($withoutExt))->exists()) {
            ...
            return response()
                ->json(['status' => 400, 'message' => 'You Must upload type file']);
        }
    }
    ...
    public function get_file_by_name(Request $request)
    {
        if ($request->has('name')) {
            ...
            if ($file) {
                ...
                return response()
                    ->json(['status' => 200, 'message' => 'No Matching File']);
            }
        } else {
            return response()
    ...
    public function delete_file_by_name(Request $request)
    {
        if ($request->has('name')) {
            ...
            if ($file) {
                ...
                return response()
                    ->json(['status' => 200, 'message' => 'file does not exist']);
            }
        } else {
            return response()
                ->json(['status' => 404, 'message' => 'input file name can not be empty']);
